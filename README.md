...
# 아무것도 지정하지 않으면 터미널에서 형식·경로·교번·비번을 차례로 물어봅니다
python3 scripts/fetch_career.py
```

비번은 화면에 표시되지 않습니다. 로그인 후 강의경력을 조회·파싱해 **선택한 형식(JSON / Markdown / 둘 다)**으로 저장합니다.

| 옵션 | 설명 |
|------|------|
| `--format {json,md,both}` | 출력 형식 (미지정 시 터미널에서 선택) |
| `--out PATH` | 출력 경로 (미지정 시 터미널에서 입력). `both`면 같은 이름으로 `.json`·`.md` 둘 다 저장 |
...
> 출처: 단국대학교 webinfo 강의경력조회 (findLctCrerInq.do) · 추출일 2026-06-17 · 총 231건 (2004~2025)

| # | 년도 | 학기 | 소속부서 | 직급 | 담당과목 | 강의시작 | 강의종료 | 시간 |
|---|------|------|----------|------|----------|----------|----------|------|
| 1 | 2025 | 2학기 | 교육대학원 | 교수 | 중도중복장애학생교육연구 | 2025-09-01 | 2026-02-28 | 3 |
```
